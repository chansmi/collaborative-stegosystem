...
def train_overseer(num_epochs=10, batch_size=32):
    ...
    for epoch in range(num_epochs):
        ...
        for _ in range(batch_size):
            ...
            overseer_prediction = overseer_outputs.logits[:, -1, :].mean().unsqueeze(0)  # Ensure it's a 1D tensor

            # Prepare target
            target = torch.tensor([float(secret_bit)])  # Create a 1D tensor

            # Compute loss
            overseer_loss = torch.nn.functional.binary_cross_entropy_with_logits(
                overseer_prediction, target
            )
